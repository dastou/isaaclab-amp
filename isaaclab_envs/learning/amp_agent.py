...
"""Agent AMP pour Isaac Lab.

Agent PPO+AMP pour apprentissage de mouvements depuis démonstrations.
Combine PPO (Proximal Policy Optimization) avec discriminateur adversarial.
"""
...
import isaaclab_envs.learning.replay_buffer as replay_buffer
import isaaclab_envs.learning.common_agent as common_agent

from tensorboardX import SummaryWriter


class AMPAgent(common_agent.CommonAgent):
    """Agent AMP pour apprentissage par imitation adversariale.

    Combine PPO avec discriminateur pour apprendre depuis démonstrations
    de mouvement (motion capture data).

    Le discriminateur apprend à distinguer les mouvements de l'agent
    des mouvements de démonstration, et fournit une récompense basée
    sur cette distinction.
    """

    def __init__(self, base_name, params):
        """Initialise l'agent AMP.
        ...
        # Normalisation des observations AMP
    ...
    def init_tensors(self):
        """Initialise les tensors de l'agent."""
        super().init_tensors()
        self._build_amp_buffers()
        return

    def set_eval(self):
        """Met l'agent en mode évaluation."""
        super().set_eval()
        if self._normalize_amp_input:
    ...
    def set_train(self):
        """Met l'agent en mode entraînement."""
        super().set_train()
        if self._normalize_amp_input:
    ...
    def get_stats_weights(self):
        """Récupère les poids des statistiques pour sauvegarde.

        Returns:
            Dictionnaire contenant les state_dict des statistiques
        """
    ...
    def set_stats_weights(self, weights):
        """Charge les poids des statistiques depuis un checkpoint.

        Args:
            weights: Dictionnaire contenant les state_dict
        """
    ...
    def play_steps(self):
        """Collecte des trajectoires en interagissant avec l'environnement.

        Returns:
            Dictionnaire contenant les données du batch collecté
        """
        self.set_eval()
    ...
    def prepare_dataset(self, batch_dict):
        ...
        Args:
            batch_dict: Dictionnaire contenant les données du batch
        """
    ...
    def calc_gradients(self, input_dict):
        """Calcule les gradients et met à jour les poids.

        Args:
            input_dict: Dictionnaire contenant les données du minibatch
        """
        self.set_train()
    ...
    def _disc_loss(self, disc_agent_logit, disc_demo_logit, obs_demo):
        ...
        Args:
            disc_agent_logit: Logits du discriminateur sur observations agent
            disc_demo_logit: Logits du discriminateur sur observations démo
            obs_demo: Observations de démonstration (pour gradient penalty)

        Returns:
            Dictionnaire contenant les métriques du discriminateur
        """
        # Perte de prédiction
    ...
    def _disc_loss_neg(self, disc_logits):
        """Perte pour observations agent (label = 0).

        Args:
            disc_logits: Logits du discriminateur
    ...
    def _disc_loss_pos(self, disc_logits):
        """Perte pour observations démo (label = 1).
    ...
    def _compute_disc_acc(self, disc_agent_logit, disc_demo_logit):
        """Calcule la précision du discriminateur.

        Args:
            disc_agent_logit: Logits sur observations agent
            disc_demo_logit: Logits sur observations démo

        Returns:
            Tuple (agent_acc, demo_acc)
        """
    ...
    def _fetch_amp_obs_demo(self, num_samples):
        """Récupère des observations de démonstration depuis l'environnement.

        Args:
            num_samples: Nombre d'échantillons à récupérer

        Returns:
            Tensor d'observations de démonstration
        """
        amp_obs_demo = self.vec_env.env.fetch_amp_obs_demo(num_samples)
        return amp_obs_demo

    def _build_amp_buffers(self):
        """Construit les buffers pour AMP."""
    ...
    def _init_amp_demo_buf(self):
        """Initialise le buffer de démonstration AMP."""
    ...
    def _update_amp_demos(self):
        """Met à jour le buffer de démonstration avec de nouveaux échantillons."""
    ...
    def _preproc_amp_obs(self, amp_obs):
        """Pré-traite les observations AMP.

        Args:
            amp_obs: Observations AMP
    ...
    def _combine_rewards(self, task_rewards, amp_rewards):
        """Combine les récompenses de tâche et du discriminateur.

        Args:
            task_rewards: Récompenses de la tâche
            amp_rewards: Dictionnaire contenant les récompenses du discriminateur

        Returns:
            Récompenses combinées
        """
    ...
    def _eval_disc(self, amp_obs):
        ...
        Returns:
            Logits du discriminateur
        """
        proc_amp_obs = self._preproc_amp_obs(amp_obs)
        return self.model.a2c_network.eval_disc(proc_amp_obs)

    def _calc_amp_rewards(self, amp_obs):
        """Calcule les récompenses AMP.

        Args:
            amp_obs: Observations AMP

        Returns:
            Dictionnaire contenant les récompenses du discriminateur
        """
    ...
    def _calc_disc_rewards(self, amp_obs):
        """Calcule les récompenses du discriminateur.

        Args:
            amp_obs: Observations AMP

        Returns:
            Récompenses du discriminateur
        """
        with torch.no_grad():
    ...
    def _store_replay_amp_obs(self, amp_obs):
        """Stocke les observations AMP dans le replay buffer.

        Args:
            amp_obs: Observations AMP à stocker
        """
        buf_size = self._amp_replay_buffer.get_buffer_size()
        buf_total_count = self._amp_replay_buffer.get_total_count()
        if (buf_total_count > buf_size):
            keep_probs = torch.tensor([self._amp_replay_keep_prob] * amp_obs.shape[0], device=self.ppo_device)
    ...
    def _record_train_batch_info(self, batch_dict, train_info):
        """Enregistre les informations du batch pour logging.

        Args:
            batch_dict: Dictionnaire contenant les données du batch
            train_info: Dictionnaire contenant les métriques d'entraînement
        """
        train_info['disc_rewards'] = batch_dict['disc_rewards']
        return

    def _log_train_info(self, train_info, frame):
        """Log les informations d'entraînement.

        Args:
            train_info: Dictionnaire contenant les métriques
            frame: Numéro de frame actuel
        """
    ...
    def _amp_debug(self, info):
        """Debug AMP (affiche les prédictions du discriminateur).
